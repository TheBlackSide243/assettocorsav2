...
-- called when any session ends
function onEndSession(encodedResultsFolderAbsolutePath, encodedResultsFileName)
    -- Decode block, you probably shouldn't touch these!
    local resultsFolderAbsolutePath = json.decode(encodedResultsFolderAbsolutePath)
    local resultsFileName = json.decode(encodedResultsFileName)

    -- Function block NOTE: this hook doesn't block, so it can run constantly alongside the manager if required

    -- Open the event results file
    --local encodedResultsJson = utils.jsonOpen(resultsFolderAbsolutePath, resultsFileName)

    -- Decode the event results file
    --success = pcall(function()
    --    resultsJson = json.decode(encodedResultsJson)
    --end)

    --if not success then
    --    print("events.lua: Couldn't decode results file: ", resultsFolderAbsolutePath .. resultsFileName)
    --    return nil
    --end

    -- Print out the results structure and data
    --print("Results:", utils.dump(resultsJson))
end
...
function onEventStart(encodedRaceConfig, encodedServerOpts, encodedEntryList)
    ...
    -- NOTE: ACSM now implements Live Weather directly, so this lua plugin is defunct. It will be left here as an example.
